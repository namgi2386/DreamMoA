...
const VideoRoom = () => {
  ...
  // OpenVidu 객체
  const OV = new OpenVidu(); // ⚠️ 문제점: useRef로 관리되어야 함

  // 사용자 입력 관련 상태
  const [mySessionRoomName, setMySessionName] = useState(''); // 방 이름
  ...
  const createSession = async (sessionId) => {
    try {
      ...
      console.log('Session creation error:', error); // ⚠️ 문제점: 에러 처리 개선 필요
      throw error;
    }
  };
  // 토큰 생성 함수
  ...
  const getToken = async () => {
    const sessionId = await createSession(mySessionRoomName);
      ...
      {!session ? (
            ...
            <form onSubmit={(e) => {
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={(e) => setMySessionName(e.target.value)}
                />
              </div>
              <button
                type="submit"
        ...
        <div className="h-full">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-xl">세션: {mySessionRoomName}</h1>
