...
# ✅ 모델 예측 함수 가져오기
from models.predict import predict_focus  # 🔥 preprocess_input 제거 (predict_focus에서 자동 처리)

# ✅ 테스트 데이터 (🔥 실제 학습 데이터와 동일한 구조)
TEST_PAYLOAD =     {
...
def test_model_prediction():
    ...
    # ✅ 모델 예측 실행 (🔥 전처리는 predict_focus 내부에서 수행)
    prediction, confidence = predict_focus(TEST_PAYLOAD)

    print(f"📡 모델 예측 결과: {prediction} (확률: {confidence:.4f})")

    # ✅ [점검] 예측값이 0 또는 1인지 확인
    assert prediction in [0, 1], "🚨 예측값이 0 또는 1이 아님!"
